...
#' @param test_n Integer. Test nÂ° identifier. Used to save png to different file.
...
r.sample.test <- function(data, times, test_n, test_type) {
  ...
  plot(as.list(appearances), data, xlab=str_interp("NÂ° apariciones en sample(${data.string}, 1)"), ylab=data.string, main=str_interp("Prueba de distribuciÃ³n de sample de R (${times} veces)."))
  dev.off()
}
