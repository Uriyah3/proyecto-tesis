...
plot.compare.kmedoids <- function(nsga_results, kobj_exp, kobj_bio) {
  ...
  ggplot() +
    ...
    geom_point(data=d2, mapping=aes(x=kobj_exp, y=kobj_bio, color="K-medoides"), size=3) + xlab("Objetivo expresi�n") + ylab("Objetivo biol�gico") + ggtitle("Posicionar soluci�n de k-medoides en la frontera\n de Pareto del algoritmo") + labs(color = "Soluci�n") +theme(text = element_text(size=15))
...
create.heatmap.for.best.solution <- function(dataset.key, biological.identifier) {
  ...
  message("Cargando mejor soluci�n")
...
boxplot.kegg.vs.moc <- function() {
  ...
  for(i in 1:5) {
    ...
    par(mgp=c(3,2,0), cex.lab=1.2, cex.axis=1.2, cex.main=1.3)
    boxplot(data, ylab='Silueta m�xima', ylim=c(0,0.76), main=paste0(dataset$name, ' Silueta m�xima'), col=c('lightblue', 'gray')) # xlab = 'Algoritmo'
    dev.off()
  }
}